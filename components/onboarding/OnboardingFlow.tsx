...
export function OnboardingFlow() {
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-8 relative">
      ...
      {/* Main card panel */}
      <GlowCard glowColor="rgba(139, 92, 246, 0.12)" className="border-white/10 p-8 shadow-2xl relative overflow-hidden bg-slate-900/80">
        ...
        {step === 1 && (
          <div className="space-y-6 animate-fade-in">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-slate-400">Select the active portals you want ReplyIQ to monitor for incoming queries.</p>
            </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div
                ...
                className={`p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${
                  channels.email 
                    ...
                    : 'border-white/5 bg-slate-950/40 hover:bg-slate-900/50'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg border ${channels.email ? 'text-blue-400 border-blue-500/20 bg-blue-500/10' : 'text-slate-500 border-white/5'}`}>
                    <Mail size={18} />
                  </div>
                  <div>
                    <h3 className="text-xs font-bold text-white">Email Gateway</h3>
                    <p className="text-[10px] text-slate-500 mt-0.5">Integrate support inbox (Gmail, Outlook, IMAP).</p>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg border ${channels.whatsapp ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/10' : 'text-slate-500 border-white/5'}`}>
                    <PhoneCall size={18} />
                  </div>
                  <div>
                    <h3 className="text-xs font-bold text-white">WhatsApp Business</h3>
                    <p className="text-[10px] text-slate-500 mt-0.5">Automate WhatsApp conversations via official API gateway.</p>
                  </div>
                </div>
                <div className={`h-5 w-5 rounded-full border flex items-center justify-center transition-all ${
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg border ${channels.sms ? 'text-cyan-400 border-cyan-500/20 bg-cyan-500/10' : 'text-slate-500 border-white/5'}`}>
                    <MessageSquare size={18} />
                  </div>
                  <div>
                    <h3 className="text-xs font-bold text-white">SMS Gateway</h3>
                    <p className="text-[10px] text-slate-500 mt-0.5">Route cell messages using Twilio integration.</p>
                  </div>
                </div>
                <div className={`h-5 w-5 rounded-full border flex items-center justify-center transition-all ${
        ...
        {step === 2 && (
          <div className="space-y-6 animate-fade-in">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-slate-400">Configure the name, tone profile, and base directives for your autonomous agent.</p>
            </div>

            <div className="space-y-4 pt-2">
              {/* Agent Name input */}
              <div className="space-y-1.5">
                <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Agent Display Name</label>
                <input
                  type="text"
                  value={agentName}
                  onChange={(e) => setAgentName(e.target.value)}
                  placeholder="e.g. ReplyIQ Copilot"
                  className="w-full px-3 py-2 rounded-lg border border-white/10 bg-slate-950/40 text-xs text-white placeholder-slate-500 outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 transition-all"
                />
              </div>

              {/* Tone Selection grid */}
              <div className="space-y-2">
                <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Voice Tone Profile</label>
                  ...
                  {tones.map((t) => (
                    <div
                      ...
                      className={`p-2.5 rounded-lg border cursor-pointer transition-all ${
                        tone === t.id
                          ? 'border-purple-500/40 bg-purple-500/5 shadow'
                          : 'border-white/5 bg-slate-950/40 hover:bg-slate-900/50'
                      }`}
                    >
                      <span className="text-xs">{t.icon}</span>
                      <span className="text-xs font-semibold text-white ml-1.5">{t.label}</span>
                      <p className="text-[9px] text-slate-500 mt-1 leading-normal">{t.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Custom Agent Directives</label>
                <textarea
                  value={instructions}
                  onChange={(e) => setInstructions(e.target.value)}
                  placeholder="e.g. Always offer a 10% discount to angry clients..."
                  className="w-full px-3 py-2 rounded-lg border border-white/10 bg-slate-950/40 text-xs text-white placeholder-slate-500 outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 transition-all resize-none min-h-[60px]"
                />
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-6 text-center py-4 animate-fade-in">
            <div className="flex justify-center">
              <div className="h-16 w-16 rounded-2xl bg-gradient-to-tr from-pink-500 via-purple-600 to-indigo-500 flex items-center justify-center border border-white/10 shadow-xl shadow-purple-500/25 relative overflow-hidden animate-pulse">
            ...
            <div className="space-y-2 max-w-sm mx-auto">
              ...
              <p className="text-xs text-slate-400 leading-relaxed font-light">
                Your AI agent <span className="font-semibold text-purple-400">{agentName}</span> is online with a <span className="font-semibold text-purple-400">{tone}</span> tone profile.
              </p>
            ...
            <div className="pt-6 flex justify-center">
              <Button
                ...
                className="flex items-center gap-2 px-8 py-2.5 text-sm font-semibold cursor-pointer shadow-lg shadow-purple-500/20"
              >
