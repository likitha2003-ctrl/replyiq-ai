...
export function LightningSplit({
	...
	const defaultLeftComponent = leftComponent || (
		<div className="flex min-h-full min-w-full h-full w-full items-center justify-center bg-blue-950">
			<h1 className="text-6xl font-bold text-white">A</h1>
		</div>
	)

	const defaultRightComponent = rightComponent || (
		<div className="flex min-h-full min-w-full h-full w-full items-center justify-center bg-purple-950">
	...
	return (
		<motion.div
			ref={containerRef}
			className="relative min-h-[100vh] w-full overflow-hidden select-none border-y border-white/10"
			...
			>
				<div className="h-8 w-[120vw] -translate-x-16 translate-y-2">
					<div className="pointer-events-none relative h-full w-full opacity-[90]">
						<div className="pointer-events-none absolute inset-0 z-20 h-full w-[100vw] translate-x-[10%] -translate-y-[48%] scale-150 lg:w-full lg:translate-x-0">
							<ShaderCanvas className="pointer-events-none h-screen w-[200vw]" />
						</div>
					</div>
				</div>
			</motion.div>
