...
export default function InboxPage() {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-slate-950/20 md:rounded-xl border border-white/5 md:shadow-2xl flex-col md:flex-row">
      ...
      <div className={`w-full md:w-[300px] shrink-0 border-r border-white/5 bg-slate-900/40 flex-col ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
        <ConversationList />
      </div>

      {/* Col 2: Chat Thread Message Area (Flex) */}
      <div className={`flex-1 flex-col bg-slate-950/60 min-w-0 relative ${activeConversation ? 'flex' : 'hidden md:flex'}`}>
        <ConversationThread />
      </div>

      {/* Col 3: AI Assistant Insights Pane (280px) */}
      {activeConversation ? (
        <div className="hidden xl:flex w-[280px] border-l border-white/5 bg-slate-950/40 p-5 flex-col gap-5 overflow-y-auto shrink-0 animate-slide-in-right">
          ...
          <div className="flex items-center gap-2 border-b border-white/5 pb-4 shrink-0">
            ...
            <div>
              ...
              <span className="text-[9px] text-slate-500 block">Real-time discussion scanner</span>
            </div>
          </div>

          {/* Copilot Ambient Status (Orb) */}
          <div className="flex flex-col items-center justify-center p-4 border border-white/5 bg-slate-900/20 rounded-xl shrink-0 shadow-[inset_0_0_20px_rgba(139,92,246,0.02)]">
            <AIOrb size="md" />
        ...
        <div className="hidden xl:flex w-[280px] border-l border-white/5 bg-slate-950/40 p-6 flex-col items-center justify-center text-center shrink-0">
          <div className="h-10 w-10 rounded-xl border border-white/5 bg-white/5 flex items-center justify-center text-slate-600 mb-3">
            <Sparkles size={16} />
          </div>
